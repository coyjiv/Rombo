...
export default async function handler( req:NextApiRequest, res: NextApiResponse,){
    ...
    if(req.method === "POST"){
        ...
        if(user?.friends.includes(potentialFriend._id)) return res.status(400).json({error:"You are already friends with this user"})
        ...
        return res.status(200).json({data:potentialFriend})
